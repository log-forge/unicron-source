"""PostgreSQL async database connection for alert-engine service.

Note: This connects to the SAME PostgreSQL database as Central.
The alerting schema was created in Phase 1 migrations - we do NOT
...
logger = get_logger("alert-engine.core.database")
...
async def session_ctx() -> AsyncGenerator[AsyncSession, None]:
    """Context manager for non-FastAPI contexts (e.g., background tasks)."""
    if _session_maker is None:
        raise RuntimeError("Database not initialized. Call init_database() first.")
    async with _session_maker() as session:
        yield session
