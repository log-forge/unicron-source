...
if not _HAS_FCNTL:
    raise RuntimeError("Herald scheduler locking requires Linux/POSIX (fcntl). " "Run Herald in Docker/WSL/Linux.")


def _default_lock_path(name: str = "herald_scheduler.lock") -> str:
    # Prefer POSIX /tmp if present (for containers), otherwise system temp dir
    posix_tmp = "/tmp"
