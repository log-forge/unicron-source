...
func retryBootstrapStep(step string, attempts int, initialDelay time.Duration, fn func() error) error {
	unlimitedAttempts := attempts <= 0
	if !unlimitedAttempts && attempts < 1 {
		attempts = 1
	}
	delay := initialDelay
	if delay <= 0 {
	...
	var lastErr error
	for attempt := 1; unlimitedAttempts || attempt <= attempts; attempt++ {
		if err := fn(); err == nil {
			if attempt > 1 {
				...
				if unlimitedAttempts {
					fields["attempts"] = "unlimited"
				}
				logrus.WithFields(fields).Info("[Bootstrap] Step recovered after retry")
			}
			return nil
		} else {
			lastErr = err
		...
		if !unlimitedAttempts && attempt == attempts {
			break
		}
		...
		if unlimitedAttempts {
			fields["max"] = "unlimited"
		}
		logrus.WithFields(fields).Warn("[Bootstrap] Step failed, retrying")
		time.Sleep(delay)
...
func BootstrapAgent(cfg BootstrapConfig) error {
	...
	if err := retryBootstrapStep("fetch_root_ca", 0, 1*time.Second, func() error {
		return fetchRootCA(cfg)
	}); err != nil {
	...
	if err := retryBootstrapStep("bootstrap_certificate", 0, 1*time.Second, func() error {
		return bootstrapCertificate(cfg, csrPEM)
	}); err != nil {
